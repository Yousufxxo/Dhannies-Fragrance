...
// Track how many items are visible per grid
const gridState = { 'home-grid': 6, 'shop-grid': Infinity };
...
function renderGrid(containerId, skeletonId) {
  ...
  const limit = gridState[containerId] ?? Infinity;
  ...
  if (!list.length) {
    ...
    document.getElementById(containerId + '-show-more')?.remove();
    return;
  }
  ...
  // Handle show-more button (only for home-grid)
  let showMoreBtn = document.getElementById(containerId + '-show-more');
  if (containerId === 'home-grid') {
    if (hasMore) {
      if (!showMoreBtn) {
        showMoreBtn = document.createElement('div');
        showMoreBtn.id = containerId + '-show-more';
        showMoreBtn.className = 'show-more-wrap';
        showMoreBtn.innerHTML = `
          <button class="btn-show-more" onclick="showMoreProducts('${containerId}')">
            <span>Show More</span>
            <i class="fas fa-chevron-down"></i>
            <span class="show-more-count">${list.length - limit} more</span>
          </button>`;
        container.parentNode.insertBefore(showMoreBtn, container.nextSibling);
      } else {
        showMoreBtn.querySelector('.show-more-count').textContent = `${list.length - limit} more`;
...
function showMoreProducts(containerId) {
  // Remove limit → show all
  gridState[containerId] = Infinity;
  renderGrid(containerId, null);
  // Hide button
  document.getElementById(containerId + '-show-more')?.remove();
  // Scroll to where new items start
  const container = document.getElementById(containerId);
  if (container) {
    const cards = container.querySelectorAll('.product-card');
    if (cards[5]) cards[5].scrollIntoView({ behavior: 'smooth', block: 'nearest' });
  }
}
...
/* ================================================================
   13. ADMIN AUTH — Supabase Email/Password Authentication
   ================================================================ */
...
async function adminLogin() {
  ...
  try {
    ...
    if (userEl) userEl.textContent = currentAdminUser.email;
    ...
    if (errEl) {
      errEl.innerHTML = '<i class="fas fa-exclamation-circle"></i> ' + (err.message || 'Invalid credentials');
...
/** Restore session on page reload (Supabase persists auth in localStorage) */
async function checkExistingSession() {
  ...
  try {
    ...
    if (session && session.user) {
      ...
      loggedIn = true;
      // Don't auto-show admin; let user navigate there
...
async function adminForgotPassword() {
  ...
  if (!supabaseClient) { showToast('DB not connected'); return; }
